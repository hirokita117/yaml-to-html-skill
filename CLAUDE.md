...
- `skills/generate-explainer-yaml/` — produces `core.yaml` + `view.yaml` + `quiz.yaml`
  (no scripts). `quiz.yaml` and the quiz view are generated by default; users opt out by
  explicitly declining a quiz in their prompt.
