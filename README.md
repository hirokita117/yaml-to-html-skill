...
A third file, `quiz.yaml` (schema:
`generate-explainer-yaml/references/quiz-yaml-schema.md`), derives comprehension-check
questions from `core.yaml` — choices, answers, explanations, difficulty — and drives the
interactive quiz tab. It is produced by default; say so in your prompt if you don't want
a quiz.
  ...
  skills/
    generate-explainer-yaml/        Skill A — Gen YAML
      ...
      references/
        ...
        quiz-yaml-schema.md         comprehension quiz schema (quiz/v1)
        sample-core.yaml            sample meaning (a PR)
        sample-view.yaml            sample strategy (engineer)
        sample-quiz.yaml            sample quiz (all five item types)
    ...
    generate-explainer-html/        Skill B — Gen HTML
      ...
      references/
        ...
        quiz-view-rules.md          rules for the interactive quiz view
        ...
        sample-quiz-iframe.html     sample interactive quiz view
...
`views.json`, copies `core.yaml` / `view.yaml` / `quiz.yaml` in, and renders the prompt
cards. It substitutes `{{core_yaml_path}}` / `{{view_yaml_path}}` / `{{quiz_yaml_path}}`
(the **absolute paths** of the copied-in YAML) into prompt bodies; any other `{{...}}`
token (e.g. `{{希望する表現}}`) is left for the user.
...
(recommended); `--view-html "LABEL=PATH"` is repeatable. Re-running with a new `--view-html` **appends** a
...
explanation (worktree + reading order + review checklist) plus an interactive quiz tab,
with a light/dark toggle and a view switcher. Left pane: copyable "add a view" templates
and the YAML viewers.
